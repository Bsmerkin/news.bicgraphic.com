...
class SalesforceIntegration extends CrmAbstractIntegration
{
    ...
    {
        return sprintf('%s/services/data/v63.0/sobjects', $this->keys['instance_url']);
    }

    public function getQueryUrl(): string
    {
        return sprintf('%s/services/data/v63.0', $this->keys['instance_url']);
    }
    ...
    /**
     * Prepare OAuth2 response for extraction, calculating token expiry.
     *
     * @return mixed
     */
    public function prepareResponseForExtraction($data)
    {
        // Some OAuth providers return expires_in. Salesforce does NOT normally include it
        // in standard OAuth responses (it returns issued_at instead). We therefore:
        // 1. If expires_in is present, trust it.
        // 2. Else if issued_at present, assume default 7200s (2h) lifetime and derive expires_at.
        // 3. Only set expires_at if not already set so we don't overwrite a future enhancement.
        if (is_array($data)) {
            if (isset($data['expires_in'])) {
                $data['expires_at'] = time() + (int) $data['expires_in'];
            } elseif (!isset($data['expires_at']) && isset($data['issued_at'])) {
                // Salesforce's issued_at is a string millisecond timestamp.
                $issuedAt = (string) $data['issued_at'];
                // Normalize to seconds (strip non-digits just in case, then handle ms length)
                $issuedAtNumeric = (int) preg_replace('/[^0-9]/', '', $issuedAt);
                if ($issuedAtNumeric > 2000000000) { // likely milliseconds
                    $issuedAtNumeric = (int) floor($issuedAtNumeric / 1000);
                }
                // Default Salesforce access token lifetime (can vary per org; 7200 is common default)
                $defaultTtl = 7200;
                $data['expires_at'] = $issuedAtNumeric + $defaultTtl;
            }
        }
                ...
                if ($dataObject) {
                    ...
                    switch ($object) {
                        ...
                        case 'Lead':
                            ...
                            // Explicitly set mailsallowed field to ensure it syncs correctly
                            // This is needed because updateBlanks setting may interfere with normal field sync
                            if ($entity && isset($dataObject['Mails_Allowed__c__Contact'])) {
                                $currentValue = $entity->getFieldValue('mailsallowed');
                                $newValue = $dataObject['Mails_Allowed__c__Contact'];
                                if ($currentValue != $newValue) {
                                    $entity->addUpdatedField('mailsallowed', $newValue);
                                }
                            }
                    ...
                    if (method_exists($entity, 'isNewlyCreated') && $entity->isNewlyCreated()) {
                        ++$created;
                        // Check both HasOptedOutOfEmail (standard) and Mails_Allowed__c (custom) for DNC status
                        $optedOut = $record['HasOptedOutOfEmail'] ?? false;
                        // Note: Mails_Allowed__c=false means NOT allowed (opted out)
                        // Check for false, 0, "0", or "false" to handle various Salesforce response formats
                        if (isset($record['Mails_Allowed__c']) && !$record['Mails_Allowed__c']) {
                            $optedOut = true;
                        }
                        
                        if (isset($record['HasOptedOutOfEmail']) || isset($record['Mails_Allowed__c'])) {
                        ...
                        ++$updated;
                        // Track DNC updates for existing contacts too (not just new ones)
                        // Check both HasOptedOutOfEmail (standard) and Mails_Allowed__c (custom) for DNC status
                        $optedOut = $record['HasOptedOutOfEmail'] ?? false;
                        // Note: Mails_Allowed__c=false means NOT allowed (opted out)
                        // Check for false, 0, "0", or "false" to handle various Salesforce response formats
    ...
    public function pushLeadDoNotContactByDate($channel, &$sfRecords, $sfObject, $params = []): void
    {
        ...
        // Early return only if we have no records at all
        // We now process DNC updates even without mauticContactIsContactableByEmail or updateDncByDate
        // because we're tracking opted_out status directly from Salesforce fields
        if (empty($sfRecords)) {
            return;
        }
        
        // Check if we should skip the history-based DNC processing later
        $skipHistoryProcessing = !isset($sfRecords['mauticContactIsContactableByEmail']) && !$this->updateDncByDate();
        ...
        foreach ($sfRecords as $record) {
            ...
            // Process DNC for newly created contacts immediately
            if (isset($record['opted_out']) && $record['opted_out'] && isset($record['is_new']) && $record['is_new']) {
                $DNCCreatedContacts[] = $record['internal_entity_id'];
            }
            
            // Process DNC for existing contacts immediately if opted_out status is set
            // This handles cases where updateDncByDate is disabled or no SF history exists
            if (isset($record['opted_out']) && isset($record['is_new']) && !$record['is_new']) {
                if ($record['opted_out']) {
                    $DNCCreatedContacts[] = $record['internal_entity_id'];
                } else {
                    // Contact is now contactable - remove from DNC if present
                    $this->doNotContact->removeDncForContact($record['internal_entity_id'], 'email', true);
                }
            }
        }
        ...
        // Skip history-based processing if the required settings aren't enabled
        if ($skipHistoryProcessing) {
            return;
        }
